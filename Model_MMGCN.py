...
class GCN(torch.nn.Module):
    def __init__(self, edge_index, batch_size, num_user, num_item, dim_feat, dim_id, aggr_mode, concate, num_layer, has_id, dim_latent=None):
        ...
        if self.dim_latent:
            ...
            self.preference = nn.init.xavier_normal_(torch.rand((num_user, self.dim_latent), requires_grad=True)).to(torch.device("cpu"))
            self.MLP = nn.Linear(self.dim_feat, self.dim_latent)
            self.conv_embed_1 = BaseModel(self.dim_latent, self.dim_latent, aggr=self.aggr_mode)
            nn.init.xavier_normal_(self.conv_embed_1.weight)
            self.linear_layer1 = nn.Linear(self.dim_latent, self.dim_id)
            # self.linear_layer1 = nn.Linear(self.dim_latent, self.num_user)
            ...
            self.preference = nn.init.xavier_normal_(torch.rand((num_user, self.dim_feat), requires_grad=True)).to(torch.device("cpu"))
    ...
    def forward(self, features, id_embedding):
        ...
        # print("self.pref.shape = ", self.preference.shape)
        # print("temp.feat.shape = ", temp_features.shape)
        x = torch.cat((self.preference, temp_features),dim=0)
        # print("x.shape = ", x.shape)
        x = F.normalize(x).to(torch.device("cpu"))

        h = F.leaky_relu(self.conv_embed_1(x, self.edge_index))#equation 1
        # print(x.shape, self.linear_layer1(x).shape, id_embedding.shape, self.has_id)
        # print("self.num_user", self.num_user)
        # print("self.num_item", self.num_item)
        # print("self.num_layer", self.num_layer)
        # print("self.dim_id", self.dim_id)
...
class Net(torch.nn.Module):
    def __init__(self, v_feat, a_feat, t_feat, words_tensor, edge_index, batch_size, num_user, num_item, aggr_mode, concate, num_layer, has_id, user_item_dict, reg_weight, dim_x):
        ...
        self.weight = torch.tensor([[1.0],[-1.0]]).to(torch.device("cpu"))
        self.reg_weight = reg_weight
        
        self.edge_index = torch.tensor(edge_index).t().contiguous().to(torch.device("cpu"))
        self.edge_index = torch.cat((self.edge_index, self.edge_index[[1,0]]), dim=1)
        self.num_modal = 0

        self.v_feat = torch.tensor(v_feat,dtype=torch.float).to(torch.device("cpu"))
        ...
        self.a_feat = torch.tensor(a_feat,dtype=torch.float).to(torch.device("cpu"))
        ...
        self.t_feat = torch.tensor(t_feat,dtype=torch.float).to(torch.device("cpu"))
        ...
        self.id_embedding = nn.init.xavier_normal_(torch.rand((num_user+num_item, dim_x), requires_grad=True)).to(torch.device("cpu"))
        self.result = nn.init.xavier_normal_(torch.rand((num_user+num_item, dim_x))).to(torch.device("cpu"))
    ...
    def full_accuracy(self, val_data, step=None, topk=10):
        user_tensor = self.result[:self.num_user]
        item_tensor = self.result[self.num_user:]

        start_index = 0
        end_index = self.num_user if step==None else step
        step = 2000 if step==None else step

        all_index_of_rank_list = torch.LongTensor([])
        print("end_index, self.num_user, start_index, step: ", end_index, self.num_user, start_index, step)
        while end_index <= self.num_user and start_index < end_index:
            temp_user_tensor = user_tensor[start_index:end_index]
            score_matrix = torch.matmul(temp_user_tensor, item_tensor.t())
            ...
            _, index_of_rank_list = torch.topk(score_matrix, topk)
            print("index_of_rank_list", index_of_rank_list)
            all_index_of_rank_list = torch.cat((all_index_of_rank_list, index_of_rank_list.cpu()+self.num_user), dim=0)
            start_index = end_index
        ...
        for data in val_data:
            ...
            length += 1
            print("user", user)
            print("all_index_of_rank_list: ", all_index_of_rank_list)
            items_list = all_index_of_rank_list[user].tolist()
