...
if __name__ == '__main__':
    ...
    # print("train_edge.shape", train_edge.shape)
    # v_feat = torch.zeros(5986,2048)
    ...
    print("user_item_dict = ", user_item_dict)
    ...
    dir_str = os.path.join(os.getcwd(), 'Data', data_path)
    val_data = np.load(dir_str + '/val_sample.npy', allow_pickle=True)
    test_data = np.load(dir_str + '/test_sample.npy', allow_pickle=True)
    ...
    for epoch in range(num_epoch):
        ...
        if torch.isnan(loss):
            with open(dir_str +'/result_{0}.txt'.format(save_file), 'a') as save_file:
